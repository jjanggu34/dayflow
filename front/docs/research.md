...
## 조언 화면 AI 개인화 계획 (2026-04-27)

- 요구사항:
  - 챗 기록(일기 저장 데이터) 중 마지막 등록 날짜의 일기 내용을 기준으로 조언 생성
  - 조언 본문은 모바일 기준 최대 6줄로 제한
  - 정적 문구가 아니라 Claude API 호출 결과를 반영
- 구현 포인트:
  - `DayflowDB.diaries`에서 최신 일기 1건(날짜+생성시각 기준)을 조회
  - `DayflowChatAgent.sendMessages()` + `DayflowApiKey.get()`로 AI 응답 생성
  - UI(`advice.html`)의 날짜/헤드라인/점수/조언 본문을 동적으로 교체
  - CSS line-clamp와 텍스트 후처리(줄 수 제한)로 6줄 이내 보장
