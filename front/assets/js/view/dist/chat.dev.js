...
(function () {
  ...
  if (!D) {
    if (typeof console !== "undefined" && console.warn) {
      console.warn("DayflowEmotionChat 없음 — emotionChatData.js 로드 실패 시 내장 폴백 사용. 가능하면 chat.html에 emotionChatData.js를 chat.js보다 먼저 넣으세요.");
    }
    ...
    window.DayflowEmotionChat = {
      ...
      getIdxFromType: function getIdxFromType(type) {
        var t = type || "good";
  ...
  function SYS_PROMPT() {
    ...
    return "당신은 감정 일기 앱 '무디(Moody)'입니다. 사용자의 하루를 공감하며 감정 기록을 돕는 따뜻한 AI 친구예요.\n\n" + "오늘 사용자가 선택한 감정: " + e.name + "\n\n대화 규칙:\n" + "- 짧고 공감적으로 답변하세요 (2~4문장)\n" + "- 한국어 존댓말만 사용하세요\n" + "- 이모지를 적절히 활용하세요";
  }
  ...
  function addTyping() {
    ...
    row.innerHTML = '<div class="chat-msg__avatar" aria-hidden="true">🌙</div>' + '<div class="chat-typing__dots" aria-hidden="true">' + "<span></span><span></span><span></span></div>";
  ...
  function addAssistantOpeningGreeting() {
    ...
    wrap.innerHTML = '<section class="text-content" aria-labelledby="chat-greeting-title">' + '<div class="text-group">' + '<h2 id="chat-greeting-title">안녕하세요!<br />' + '<span>' + escapeHtml(line2) + '</span>' + "</h2>" + '<p id="chat-greeting-sub">선택하신 감정의 세부 감정을 선택해주세요</p>' + "</div>" + '<span class="chat-msg__time chat-msg__time--assistant-opening">' + formatChatTime() + "</span>" + "</section>";
  ...
  function addBotBubble(htmlOrText, asHtml) {
    ...
    if (asHtml) bubble.innerHTML = htmlOrText.replace(/\n/g, "<br>");else bubble.innerHTML = escapeHtml(htmlOrText).replace(/\n/g, "<br>");
  ...
  function addUserMessage(text) {
    ...
    wrap.innerHTML = '<div class="chat-msg__body">' + '<p class="chat-msg__bubble">' + escapeHtml(text).replace(/\r\n|\r|\n/g, "<br />") + "</p>" + '<span class="chat-msg__time">' + formatChatTime() + "</span></div>";
    chatMessages.appendChild(wrap);
    scrollThreadToBottom();
  }
  ...
  function buildChatErrorMessage(err) {
    ...
    if (err.message && /Failed to fetch|NetworkError|Load failed/i.test(err.message)) {
      ...
      if (apiUrl.indexOf("127.0.0.1:8787") !== -1 || apiUrl.indexOf("localhost:8787") !== -1) {
        return "로컬 프록시(포트 8787)에 연결되지 않았어요. 터미널에서 실행:\n" + "python3 front/scripts/anthropic_proxy.py\n" + "실행 후 페이지를 새로고침하고 다시 보내 보세요.";
      }

      return "네트워크 오류이거나 CORS 제한일 수 있어요. 로컬이면 프록시를 켜고, 배포 환경이면 백엔드 URL을 window.DAYFLOW_ANTHROPIC_URL 로 지정하세요.";
    }
  ...
  function setVoiceBannerSpeaking(speaking) {
    ...
    if (el) {
      el.textContent = speaking ? "AI 응답 재생 중… (끝나면 다시 말씀해 주세요)" : "음성 대화 중 — 말씀하신 뒤 " + VOICE_AUTO_SEND_MS / 1000 + "초 뒤 자동 전송돼요";
    }
  }
  ...
  function handleChat(text) {
    ...
    if (!window.DayflowApiKey || !DayflowApiKey.has()) {
      var needKeyMsg = "Claude와 대화하려면 상단 ⚙에서 API 키를 먼저 입력해 주세요.\n키는 이 브라우저에만 저장돼요.";
  ...
  function requestDiaryNarrativeSummary(fullText, onDone) {
    if (typeof onDone !== "function") onDone = function onDone() {};
    var t = (fullText || "").trim();
    ...
    DayflowChatAgent.sendMessages({
      apiKey: DayflowApiKey.get(),
      system: "당신은 감정 일기 앱의 편집자입니다. 아래는 사용자가 챗봇과 나눈 하루 기록입니다. " + "2~3문장 정도의 한국어 존댓말로 한 덩어리만 써 주세요. 글머리표·따옴표·제목 없이 본문만. " + "대화에 없는 사실은 지어내지 마세요.",
      messages: [{
        role: "user",
        content: "[일기 대화]\n\n" + t.slice(0, 12000)
      }]
  ...
  function finishDiary() {
    ...
    function saveThenGo() {
      if (window.DayflowDiaryStore && typeof DayflowDiaryStore.saveTodayDiary === "function") {
        ...
        }).then(goResult)["catch"](function (err) {
          if (typeof console !== "undefined" && console.warn) {
    ...
    requestDiaryNarrativeSummary(text, function (blurb) {
      try {
        if (blurb) sessionStorage.setItem(STORAGE_CHAT_SUMMARY, blurb.slice(0, 2000));else sessionStorage.removeItem(STORAGE_CHAT_SUMMARY);
      } catch (e3) {}
  ...
  function openApiKeyPrompt() {
    ...
    var k = window.prompt("Anthropic Claude API 키를 입력하세요.\n(콘솔에서 발급 — 이 브라우저 localStorage에만 저장됩니다)", cur || "");
    if (k === null) return;
    k = String(k).trim();
  ...
  if (voiceCancel) voiceCancel.addEventListener("click", function () {
    stopVoice(true);
  });
